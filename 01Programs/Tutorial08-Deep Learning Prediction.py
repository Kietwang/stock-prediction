...
def getDeepLearningData(ticker):
    # Step 1. Load data
    data = pandas.read_csv('../02Data/01.IntradayUS/'+ticker+'.csv')['close'].tolist()
    ...
    for i in range(CONST_TESTING_CASES,0,-1):
        dataSegment = data[-(i+1)*CONST_TRAINING_SEQUENCE_LENGTH:-i*
    ...
    #Step 4 Reshape for deep learning 将数据模型变换一下
...
def predict_m(model,X):
    ...
    for i in range(len(X)):
        ...
        for j in range(CONST_TRAINING_SEQUENCE_LENGTH):
            ...
            data = data[1:]
            data = numpy.insert(data,[CONST_TRAINING_SEQUENCE_LENGTH-1],predicted,
                                axis =0)
...
def predictLSTM(ticker):
    ...
    model.add(LSTM(input_dim = 1,
                   output_dim =50,
                   return_sequences=True))  #True返回全部time step 的 hidden state值  。。#False 返回单个 hidden state值
    model.add(Dropout(0.2))   #随机去掉一些点，防止overfit，dropout只是临时去掉
    ...
    model.compile(loss='mse',optimizer='rmsprop') #优化器
    ...
    # Step 4. Predict
    predictionsNormalized = predict_m(model,X_Testing)
